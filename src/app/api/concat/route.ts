...
export async function POST(req: NextRequest) {
  ...
  reqLogger.info({ event: 'request_start', route: '/api/concat' });
  if (req.method === 'POST') {
    try {
      ...
      reqLogger.info({ event: 'request_ok', route: '/api/concat' });
      ...
      reqLogger.error({ event: 'request_error', route: '/api/concat', err: error?.message ?? String(error) });
