...
export async function POST(req: NextRequest) {
  ...
  reqLogger.info({ event: 'request_start', route: '/api/generate_lyrics' });
  if (req.method === 'POST') {
    try {
      ...
      reqLogger.info({ event: 'request_ok', route: '/api/generate_lyrics' });
      ...
      reqLogger.error({ event: 'request_error', route: '/api/generate_lyrics', err: error?.message ?? String(error) });
