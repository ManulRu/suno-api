...
export async function POST(req: NextRequest) {
  ...
  reqLogger.info({ event: 'request_start', route: '/api/generate_stems' });
  if (req.method === 'POST') {
    try {
      ...
      reqLogger.info({ event: 'request_ok', route: '/api/generate_stems' });
      ...
      reqLogger.error({ event: 'request_error', route: '/api/generate_stems', err: error?.message ?? String(error) });
