...
export async function POST(req: NextRequest) {
  ...
  reqLogger.info({ event: 'request_start', route: '/api/extend_audio' });
  if (req.method === 'POST') {
    try {
      ...
      reqLogger.info({ event: 'request_ok', route: '/api/extend_audio' });
      ...
      reqLogger.error({ event: 'request_error', route: '/api/extend_audio', err: error?.message ?? String(error) });
